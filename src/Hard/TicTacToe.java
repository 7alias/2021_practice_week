...
public class TicTacToe {
    ...
    String checkGame() {
        ...
        String[] diagonal1 = {field[0][0], field[1][1], field[2][2]};
        String[] diagonal2 = {field[0][2], field[1][1], field[2][0]};

        boolean check = true;
        for (int i = 0; i < 3; i++) {
            if (diagonal1[i].equals("0") || diagonal1[i].equals("-")) check = false;
        }
        if (check) return "X";
        else {
            check = true;
            for (int i = 0; i < 3; i++) {
                if (diagonal1[i].equals("X") || diagonal1[i].equals("-")) check = false;
            }
            if (check) return "0";
        }
